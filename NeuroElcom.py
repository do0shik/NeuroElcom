...
from PyQt5.QtWidgets import (
    ...
    QScrollArea, QScrollBar, QGraphicsDropShadowEffect, QGraphicsOpacityEffect, QGraphicsScale
)
from PyQt5.QtGui import QIcon, QPixmap, QFontDatabase, QFont, QColor, QPainter, QBrush
from PyQt5.QtCore import Qt, QSize, QPropertyAnimation, QTimer, QPoint, QRect, QParallelAnimationGroup, QEasingCurve, QPropertyAnimation
...
class MainWindow(QMainWindow):
                ...
                if scroll_area:
                    ...
                    if content_widget:
                        ...
                        for child_widget in content_widget.children():
                            if isinstance(child_widget, QLabel) and child_widget.text() in categories_data:
                                current_category = child_widget.text()
                                print(f"Найдена категория: {current_category}")
                            elif isinstance(child_widget, QWidget):
                                combos = [w for w in child_widget.children() if isinstance(w, QComboBox)]
                                if len(combos) == 3 and current_category:
                                    print(f"Найдена строка с 3 комбобоксами для категории: {current_category}")
                                    row_data = []
    ...
    def build_result_page(self, w):
        ...
        # Кнопка "Создать PDF" - теперь отдельный элемент
        ...
        # Левая панель с таблицей (теперь без кнопки "Назад")
        ...
        # Создаем область прокрутки (оставляем без изменений)
        scroll = QScrollArea(left)
        scroll.setWidgetResizable(True)
        scroll.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        scroll.setGeometry(10, 10, 870, 770)
        ...
        # Создаем контейнер для содержимого (оставляем без изменений)
        content = QWidget()
        ...
        # Загрузка данных и создание таблицы (оставляем без изменений)
        ...
        for cat_name, groups in categories:
            ...
            for text, w in zip(["Артикул", "Наименование", "Цена"], widths):
                lh = QLabel(text, colhdr)
                lh.setFixedWidth(w)
                lh.setAlignment(Qt.AlignCenter)
                lh.setFont(font)
                lh.setStyleSheet("font-weight: 900; color: #B1B1B1;")
            ...
            for idx, grp in enumerate(groups, 1):
                ...
                # Синхронизация комбобоксов (оставляем без изменений)
                def make_sync(a, b, c):
                    def sync():
        ...
        # Кастомный скроллбар (оставляем без изменений)
        ...
        right_container.setGeometry(975, 120, 900, 800)
        ...
        image_with_button.setMinimumSize(600, 500)
        ...
        # Обертка, имитирующая абсолютное позиционирование кнопки в углу
        ...
        def on_press():
            ...
            icon_anim.start()
            expand_btn._icon_anim = icon_anim  # чтобы не удалился сборщиком мусора
